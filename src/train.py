...
def prepare_df(data):
    ...
    # fix missing value
    health_indicator_mode = data['health_indicator'].mode()[0]
    data['health_indicator'] = data['health_indicator'].fillna(health_indicator_mode)
    ...
    # try remove outliers and get cleaned data for training model otherwise use initial data as there is no outliers
    try:
        df_full_train_cleaned = df_full_train.drop(index=df_outliers_q99.query('feature == "reco_policy_premium_log"').iloc[:,1].tolist()[0])
    except:
        df_full_train_cleaned = df_full_train
    ...
    # ====> reco_policy_cat_mapping: response_mean >= 0.25 -> health_indicator, response_mean < 0.25 -> other
    ...
    # ====> top_city : response_mean >= 0.25 -> city_code , response_mean < 0.25 -> other
    
    df_train['city_code_group'] = np.where(df_train['city_code'].isin(list_top_city), df_train['city_code'], 'other')
    df_train['reco_policy_cat_group'] = np.where(df_train['reco_policy_cat'].isin(list_reco_policy_cat), df_train['reco_policy_cat'], 'other')
    
    df_validation['city_code_group'] = np.where(df_validation['city_code'].isin(list_top_city), df_validation['city_code'], 'other')
    df_validation['reco_policy_cat_group'] = np.where(df_validation['reco_policy_cat'].isin(list_reco_policy_cat), df_validation['reco_policy_cat'], 'other')
    
    df_test['reco_policy_premium_log'] = np.log1p(df_test['reco_policy_premium'])
    df_test['city_code_group'] = np.where(df_test['city_code'].isin(list_top_city), df_test['city_code'], 'other')
    df_test['reco_policy_cat_group'] = np.where(df_test['reco_policy_cat'].isin(list_reco_policy_cat), df_test['reco_policy_cat'], 'other')

    return [df_train, df_validation, df_test, list_top_city, list_reco_policy_cat, health_indicator_mode] 
...
# DATA PREPROCESSING
df_train, df_validation, df_test, list_top_city, list_reco_policy_cat, health_indicator_mode = prepare_df(data)
...
data_to_save = {
    ...
    'list_top_city': list_top_city,
    'list_reco_policy_cat': list_reco_policy_cat,
    'health_indicator_mode' : health_indicator_mode
}
...
y_train_pred = loaded_pipeline['model'].predict(X_full_train)
y_test_pred = loaded_pipeline['model'].predict(X_test)
